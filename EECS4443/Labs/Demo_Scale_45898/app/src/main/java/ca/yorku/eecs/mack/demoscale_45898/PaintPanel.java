...
public class PaintPanel extends View
{
    ...
    private class MyGestureListener extends GestureDetector.SimpleOnGestureListener
    {
        ...
        //added an override of onDoubleTap to handle zooming in and out for the image
        @Override
        public boolean onDoubleTap(MotionEvent e) {
            float x = e.getX();
            float y = e.getY();

            //calculate the new scaleFactor and center position
            if (!zoomedIn) {
                // Zoom in by a factor of 3 centered around the double-tap point
                xPosition = x - (x - xPosition) * 3.0f;
                yPosition = y - (y - yPosition) * 3.0f;
                scaleFactor *= 3.0f;
            } else {
                //zoom out by a factor of 1/3 centered around the double-tap point
                xPosition = x - (x - xPosition) / 3.0f;
                yPosition = y - (y - yPosition) / 3.0f;
                scaleFactor /= 3.0f;
            }

            //limit the scale factor within a reasonable range so that it can zoom out
            scaleFactor = Math.max(0.1f, Math.min(scaleFactor, 10.0f));

            //toggle the zoomedIn state
            zoomedIn = !zoomedIn;

            //trigger a re draw of the image
            invalidate();
        ...
        public boolean onDoubleTapEvent(MotionEvent e) {
            // Implement this method to handle other events related to double-tap, if needed.
            return false;
        }
    }
}
